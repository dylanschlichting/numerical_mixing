'''
Computes the volume-integrated salt squared, volume-mean salinity variance, and the cross 
horizontal diffusive within a 3D control volume for ROMS model output. 
This is intended to compare with boundary-area integrated quantities. 
...
Notes: chi_h = 2 * kappa_h * \nabla_H(s)^2
    '''
    #calculate lateral gradients using a Jacobian.
...
#Functions for diffusion terms 
def horz_s2_diff_vol(ds, etaslice, xislice, grid, k_h):
    '''
Calculates the volume-integrated horizontal volume-mean salt squared
diffusion.
    '''
...
#Subset and save to a netcdf
dates = np.arange('2010-06-03', '2010-07-14', dtype = 'datetime64[D]') 
for d in range(len(dates)):
    s2_horz_diff_parent_sel = s2_diff_vol.sel(ocean_time = str(dates[d]))
    s2_horz_diff_parent_sel.attrs = ''
    path = '/d2/home/dylan/JAMES/revised_submission/budget_recalcs/30min/hdiffusion/s2_hdiffusion_60min_%s.nc' %d
    ...
    path = '/d2/home/dylan/JAMES/revised_submission/budget_recalcs/60min/hdiffusion/sprime2_hdiffusion_60min_%s.nc' %d
    ...
    path = '/d2/home/dylan/JAMES/revised_submission/budget_recalcs/60min/hdiffusion/sbarsprime_hdiffusion_60min_%s.nc' %d
    # path = '/d2/home/dylan/JAMES/revised_submission/budget_recalcs/30min/hdiffusion/sbarsprime_hdiffusion_30min_%s.nc' %d
    # path = '/d2/home/dylan/JAMES/revised_submission/budget_recalcs/10min/hdiffusion/sbarsprime_hdiffusion_10min_%s.nc' %d
